...
def resolve_entities_for_type(names: list[str]) -> dict[str, str]:
    ...
    return {
        n: canonical_for_root[uf.find(n)]
        for n in unique_names
    }
...
if __name__ == "__main__":
    ...
    for etype, names in names_by_type.items():
        print(
            f"  {etype}: "
            f"{len(names)} raw, "
            f"{len(set(names))} unique strings"
        )
    ...
    for r in all_relations:
        resolved_relations.append(
            {
                "source_entity": rename_map[
                    (r["source_type"], r["source_entity"])
                ],
                "source_type": r["source_type"],
                "relation_type": r["relation_type"],
                "target_entity": rename_map[
                    (r["target_type"], r["target_entity"])
                ],
    ...
    print(
        f"\nSaved {len(resolved_entities)} entities, "
        f"{len(resolved_relations)} relations "
        f"to {OUTPUT_PATH}"
    )
